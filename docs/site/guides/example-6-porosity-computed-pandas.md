...
`examples/notebooks/user/computed_pandas/forge16b_porosity_example_pandas_computed.ipynb`.

It starts from the same public LAS source used by
[Example 2](example-2-porosity-reconstruction.md), but uses pandas to compute
porosity and gas-crossover channels before generating the layout.
...
- [Open the notebook on GitHub](https://github.com/cschrupp/wellplot/blob/main/examples/notebooks/user/computed_pandas/forge16b_porosity_example_pandas_computed.ipynb)
- [Download the notebook](https://raw.githubusercontent.com/cschrupp/wellplot/main/examples/notebooks/user/computed_pandas/forge16b_porosity_example_pandas_computed.ipynb)
...
This is the tabular version of the porosity workflow. It is useful when your
team prefers table-oriented calculations, QC columns, and pandas-based
transforms before plotting.
...
The saved YAML remains a layout artifact only. To reproduce the computed
channels, rerun the notebook or export the computed dataset separately.
