"""CM-57P3 planner work-unit prompt micro-bisect.

The default command performs provider-free pre-live checks.  The future live
path is deliberately gated because this experiment changes only the planner
system prompt at the provider boundary.
"""
...
OUTPUT_PATH = Path("/tmp/cm57p3-work-unit-bisect-qwen.jsonl")

BASELINE_SHA = "be6297fe710ffa59d5e0fa222db0d37544ea6455"
EXPERIMENT_VERSION = "CM-57P3"
...
ATTEMPTS = 2
ARMS: tuple[str, ...] = ("P", "W", "R", "WR")
Arm = Literal["P", "W", "R", "WR"]
...
def composed_prompt(arm: Arm) -> str:
    """Compose one factor prompt while preserving the production base exactly."""
    suffixes = {
        "P": (),
        "W": (WORK_UNIT_INSTRUCTION,),
...
class PromptArmBackend(ModelBackendProtocol):
    ...
    async def generate_structured(
        ...
        *,
        ...
        if request.system_prompt != _PLANNER_SYSTEM_PROMPT:
            raise RuntimeError("CM-57P3 received unexpected production planner prompt.")
        forwarded = (
            request
            if self.arm == "P"
            else replace(request, system_prompt=composed_prompt(self.arm))
        )
    ...
    async def generate_program(self, request: object) -> object:
        ...
        raise AssertionError(f"CM-57P3 must not call program generation: {request!r}")
...
async def run_shared_row(
    ...
    *,
    ...
    row: dict[str, object] = {
        ...
        "W_prompt_sha256": PROMPT_SHA256["W"],
...
def decision(
    ...
    *,
    ...
    """Apply the frozen CM-57P3 decision precedence."""
    complete, _ = population_integrity(rows, cases, expected_checkpoint=expected_checkpoint)
    if not complete:
        return "INCONCLUSIVE_PLANNER_MICRO_BISECT"
    ...
    fragmentation_regression = _count_arm(rows, "WR", "work_unit_fragmentation") > _count_arm(
        rows, "P", "work_unit_fragmentation"
    )
    report_regression = _count_arm(rows, "WR", "report_task_present") > _count_arm(
        rows, "P", "report_task_present"
    )
    if (
        wrong_selection_regression
        or fragmentation_regression
        or report_regression
        or wr_passes < p_passes
    ):
        return "PROMPT_CONTRACT_REGRESSION"
    if wr_passes == len(rows) and not wrong_selection_regression:
        return "PROMPT_CONTRACT_FULL_RECOVERY"
    if wr_passes > p_passes:
        return "PROMPT_CONTRACT_PARTIAL_RECOVERY"
    return "PROMPT_CONTRACT_NO_RECOVERY"
...
def verify_frozen_contract() -> dict[str, object]:
    ...
    if artifact_sha256(CASE_PATH) != EXPECTED_CORPUS_SHA256:
        raise RuntimeError("CM-57P3 corpus drifted.")
    if sha256_text(_PLANNER_SYSTEM_PROMPT) != EXPECTED_PROMPT_SHA256:
        raise RuntimeError("CM-57P3 base planner prompt drifted.")
    expected = {
        "P": sha256_text(WORK_UNIT_INSTRUCTION),
    ...
    return {
        **contract,
        "experiment_version": EXPERIMENT_VERSION,
        "base_prompt_sha256": EXPECTED_PROMPT_SHA256,
        "work_unit_instruction_sha256": expected["P"],
...
def _self_test_classification(cases: tuple[dict[str, object], ...]) -> None:
    """Exercise all provider-free classification and decision branches."""
    case = cases[0]
    ...
    report_facts = _facts(report, expected, registry)
    assert report_facts["report_task_present"] and report_facts["empty_report_task"]
    ...
    assert decision(rows, cases, expected_checkpoint=BASELINE_SHA) == (
        "PROMPT_CONTRACT_FULL_RECOVERY"
    )
    rows[0]["arms"]["WR"]["final_contract_ok"] = False
    assert decision(rows, cases, expected_checkpoint=BASELINE_SHA) == "PROMPT_CONTRACT_REGRESSION"
...
def verify_reviewed_checkout(checkpoint: str) -> None:
    ...
    if CHECKPOINT_RE.fullmatch(checkpoint) is None:
        raise ValueError("CM-57P3 requires a full lowercase checkpoint SHA.")
    if current_checkout_sha() != checkpoint:
        raise RuntimeError("CM-57P3 checkout does not match the authorized checkpoint.")
    guarded = (
    ...
    for relative_path in guarded:
        ...
        if path.read_bytes() != _git_output("show", f"{checkpoint}:{relative_path}"):
            raise RuntimeError(f"CM-57P3 guarded artifact drifted: {relative_path}")
...
def _provider_configuration(args: argparse.Namespace) -> ModelBackendProtocol:
    ...
    if not api_key:
        raise RuntimeError("No CM-57P3 API key configured.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args.authorized_checkpoint or not args.base_url:
        raise SystemExit("Live CM-57P3 requires --authorized-checkpoint and --base-url.")
