...
def author_plot_from_request_prompt(
    ...
    return (
        ...
        f"{seed_block}\n\n"
        "Workflow:\n"
        "0. Treat every MCP tool result marked `is_error: true` as a failed operation. "
        "Do not claim progress for it; correct the arguments or report the blocker.\n"
        ...
        "12a. For multi-section structure changes, fully build and verify the source "
        "section before calling replicate_section_structure(...). Add or update all "
        "requested source tracks before replication, and defer move_track(...) until "
        "the target section contains every requested track.\n"
