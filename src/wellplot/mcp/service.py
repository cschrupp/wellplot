...
def review_logfile_prompt(logfile_path: str) -> str:
    ...
    return (
        "Review this wellplot logfile through the stable MCP surface.\n\n"
        f"Logfile path: {logfile_path}\n\n"
        "Workflow:\n"
        "1. Call validate_logfile(...), using the render tier when the next step is rendering.\n"
        "2. Call inspect_authoring(...) for the section or track scope relevant to the review.\n"
        "3. Call inspect_source(...) only when source metadata or channel availability matters.\n"
        "4. Summarize validated issues, the inspected state, and the smallest next edit "
        "or preview.\n"
    )
...
def preview_logfile_prompt(logfile_path: str, focus: str | None = None) -> str:
    ...
    return (
        "Preview this wellplot logfile through the stable MCP surface.\n\n"
        f"Logfile path: {logfile_path}\n"
        f"{focus_text}\n\n"
        "Workflow:\n"
        "1. Call validate_logfile(...).\n"
        "2. Call inspect_authoring(...) to identify the relevant section or track.\n"
        "3. Call preview_logfile(...) with the relevant section, track, or page scope.\n"
        "4. Briefly explain the preview and the smallest next refinement, if any.\n"
    )
...
def start_from_example_prompt(example_id: str, goal: str) -> str:
    ...
    _production_asset_path(example_id, "README.md")
    return (
        f"Adapt the packaged wellplot production example `{example_id}` for this goal:\n"
        f"{goal}\n\n"
        "Read resources only as needed:\n"
        f"- wellplot://examples/production/{example_id}/README.md\n"
        f"- wellplot://examples/production/{example_id}/base.template.yaml\n"
        f"- wellplot://examples/production/{example_id}/full_reconstruction.log.yaml\n\n"
        "Workflow:\n"
        "1. Call create_draft(...) with the packaged example as the starter.\n"
        "2. Call inspect_authoring(...) before changing existing sections, tracks, or bindings.\n"
        "3. Use the narrow edit tool that owns each requested object.\n"
        "4. Call validate_logfile(...) and preview_logfile(...) before a final render.\n"
    )
...
def author_plot_from_request_prompt(
    ...
    return (
        ...
        f"{seed_block}\n\n"
        "Workflow:\n"
        "1. Call inspect_authoring(...) for the requested header, section, track, "
        "or binding scope.\n"
        "2. Call inspect_source(...) before choosing or binding source channels.\n"
        "3. Call inspect_vocab(...) only when the requested object family or value is unclear.\n"
        "4. Use edit_header(...), edit_remarks(...), or edit_report_settings(...) "
        "for report content and report-wide settings.\n"
        "5. Use edit_section(...) and edit_track(...) to create or change structure. Build the "
        "source section before calling replicate_section_structure(...).\n"
        "6. Use edit_curve_binding(...), edit_raster_binding(...), edit_fill(...), and "
        "edit_annotation(...) only for the matching child object.\n"
        "   Every curve or raster mutation must include logfile_path, section_id, and "
        "track_id in the same call; binding_id never replaces track_id. For raster "
        "bindings, omit sample_axis unless its label and unit are non-empty.\n"
...
def revise_plot_from_feedback_prompt(logfile_path: str, feedback: str) -> str:
    ...
    return (
        ...
        f"Feedback:\n{feedback}\n\n"
        "Workflow:\n"
        "1. Call inspect_authoring(...) for every object the feedback changes.\n"
        "2. Call inspect_source(...) before adding or rebinding source channels.\n"
        "3. Use edit_header(...), edit_remarks(...), and edit_report_settings(...) "
        "for report content or report-wide settings.\n"
        "4. Use edit_section(...), edit_track(...), and replicate_section_structure(...) "
        "for layout.\n"
...
def ingest_header_text_prompt(
    ...
    return (
        ...
        "```\n\n"
        "Workflow:\n"
        "1. Call inspect_authoring(...) with header-slot and service-title scopes to inspect the "
        "existing archetype.\n"
        "2. If the requested header family differs, call edit_header(...) with the apply-archetype "
        "operation before assigning values.\n"
        "3. Apply only unambiguous values with edit_header(...). Leave unmatched values unchanged "
        "and explain the skipped source text.\n"
        "4. If the source includes a freeform note block, add it separately with "
        "edit_remarks(...).\n"
        "5. Call validate_logfile(...) and preview_logfile(...) before the final render.\n"
    )
